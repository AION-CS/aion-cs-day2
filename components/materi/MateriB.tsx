...
const LEVERS = [
  {
    id: "A",
    name: "Intensified personal account management",
    motive: "Relationship · trust",
    shape: "fixed",
    what: "A named key account manager, a business review each quarter and early-warning contact before a renewal date. It gives the client a person who knows the estate and is accountable for it.",
    cost: "A cost per client per year: staff time is spent whether or not the client re-orders.",
    fits: "Where contact is frequent and the client keeps buying from people it knows. In German IT projects the account owner often carries the relationship through several contract cycles.",
  },
  {
    id: "B",
    name: "Discounting",
    motive: "Price",
    shape: "perOrder",
    what: "A rebate on the repeat order, for example an 8% reduction agreed in a Rahmenvertrag (frame agreement) for the next order.",
    cost: "Margin given up on every order the discount applies to. It is paid per order, not per client, so it grows with the number of repeat orders.",
    fits: "Where the reason a client leaves is price. It is a price answer to a price problem.",
  },
  {
    id: "C",
    name: "Value-added service",
    motive: "Benefit · perceived value",
    shape: "fixed",
    what: "A service the client values on top of the contract: a security health check, training for the client’s team, a monitoring add-on or an extended hypercare period.",
    cost: "A delivery cost per client per year, again independent of how many orders follow.",
    fits: "Where the client would stay for a visible, measurable extra result. It answers the benefit motive.",
  },
] as const;
  ...
  return (
    <div className="space-y-3">
      <svg viewBox="0 0 560 240" className="mx-auto h-auto w-full max-w-[600px]" role="group" aria-labelledby={`${uid}-t ${uid}-d`}>
        <title id={`${uid}-t`}>{`Three retention levers, the motive each answers and how its cost behaves`}</title>
        <desc id={`${uid}-d`}>{`Account management answers the relationship motive with a fixed cost per client. Discounting answers the price motive with a cost on every order. A value-added service answers the benefit motive with a fixed cost per client. Select a lever to read more.`}</desc>
        <text x="4" y="14" fontSize="12" fontWeight="700" fill="#59606A">LEVER</text>
        <text x="216" y="14" fontSize="12" fontWeight="700" fill="#59606A">MOTIVE IT ANSWERS</text>
        <text x="412" y="14" fontSize="12" fontWeight="700" fill="#59606A">HOW THE COST BEHAVES</text>
        {LEVERS.map((x, i) => {
          const y = 24 + i * 70;
          const on = x.id === sel;
          ...
          return (
            ...
            >
              ...
              <text x="12" y={y + 46} fontSize="13" fill="#59606A">{x.id === "A" ? "Account management" : x.id === "B" ? "Discount" : "Value-added service"}</text>
              ...
              {x.shape === "fixed" ? (
                <g>
                  ...
                  <text x="477" y={y + 35} textAnchor="middle" fontSize="12.5" fontWeight="600" fill="#1F2328">per client</text>
                </g>
              ) : (
                <g>
                  {[0, 1, 2, 3, 4].map((k) => (
                  ...
                  <text x="477" y={y + 58} textAnchor="middle" fontSize="12" fill="#59606A">per order</text>
                </g>
              )}
            </g>
          );
      ...
      <div aria-live="polite" className="space-y-2 rounded-lg border border-line bg-paper p-4 text-caption">
        ...
        <p><span className="font-semibold text-ink">What it is. </span>{l.what}</p>
...
const SEGS = [
  {
    ...
    y: 66,
    r: 44,
    what: "Large, one-off implementations. A client places few orders and each is large.",
    how: "A repeat order is the next project: rare, decided in a committee and months apart. Personal contact between projects matters, but there is little day-to-day contact to build on. A discount lands on a very large ticket.",
  },
  {
    id: "retainer",
    ...
    y: 190,
    r: 34,
    what: "Ongoing, smaller support contracts. A client places many orders and each is small.",
    how: "A repeat order is a renewal or an expansion of something already running: frequent, with regular contact. Relationship levers have material to work with, and a price cut applies to every one of many orders.",
  },
] as const;
  ...
  return (
    <div className="space-y-3">
      <svg viewBox="0 0 560 270" className="mx-auto h-auto w-full max-w-[600px]" role="group" aria-labelledby={`${uid}-t ${uid}-d`}>
        <title id={`${uid}-t`}>{`Two client segments by order frequency and order value`}</title>
        <desc id={`${uid}-d`}>{`Project clients sit at low order frequency and high value per order. Retainer clients sit at high frequency and low value per order. Select a segment to read how levers land on it.`}</desc>
        <line x1="50" x2="50" y1="12" y2="240" stroke="#59606A" strokeWidth="1.6" />
        <line x1="50" x2="548" y1="240" y2="240" stroke="#59606A" strokeWidth="1.6" />
        <text x="8" y="130" fontSize="12.5" fill="#59606A" transform="rotate(-90 14 130)" textAnchor="middle">value per order</text>
        <text x="299" y="262" fontSize="12.5" fill="#59606A" textAnchor="middle">orders per client per year</text>
        <text x="54" y="254" fontSize="12" fill="#59606A">rare</text>
        <text x="548" y="254" fontSize="12" fill="#59606A" textAnchor="end">frequent</text>
        <text x="56" y="24" fontSize="12" fill="#59606A">large</text>
        <text x="56" y="232" fontSize="12" fill="#59606A">small</text>
        {SEGS.map((x) => {
          const on = x.id === sel;
          const pick = () => setSel(x.id);
          return (
            ...
            >
              ...
              <text x={x.x + 60} y={x.y + 45} textAnchor="middle" fontSize="14.5" fontWeight="700" fill="#1F2328">{x.id === "project" ? "Project" : "Retainer"}</text>
            </g>
          );
        })}
      </svg>
...
export function FormulaChain() {
  ...
  const boxes = [
    ...
    { x: 148, w: 130, top: "× Gross profit", sub: "price × margin" },
    { x: 296, w: 130, top: "− Cost", sub: "of the option" },
    { x: 444, w: 116, top: "= Net impact", sub: "in € per year" },
  ];
  return (
    <svg viewBox="0 0 560 92" className="mx-auto h-auto w-full max-w-[600px]" role="img" aria-labelledby={`${uid}-t ${uid}-d`}>
      <title id={`${uid}-t`}>{`Net impact of a retention lever`}</title>
      <desc id={`${uid}-d`}>{`Extra orders equal clients times the uplift. Multiply by gross profit per order, which is price times margin. Subtract the cost of the option. The result is the net impact in euros per year.`}</desc>
      {boxes.map((b, i) => (
        <g key={b.top}>
  ...
  return (
    <div className="space-y-3">
      <svg viewBox="0 0 560 240" className="mx-auto h-auto w-full max-w-[600px]" role="img" aria-labelledby={`${uid}-t ${uid}-d`}>
        <title id={`${uid}-t`}>{`Net impact of a fixed-cost lever and a discount as the uplift grows`}</title>
        <desc id={`${uid}-d`}>{`Illustrative company. Net impact in euros against the repeat-rate uplift in percentage points. The fixed-cost lever starts at minus 12,000 euros. The discount starts at minus the discount on every existing repeat order and rises more slowly. With ${base} existing repeat orders the discount breaks even at ${be.toFixed(1)} percentage points; the fixed-cost lever breaks even at ${fixedBreakEven.toFixed(1)}. At ${u} percentage points the fixed-cost lever nets ${eur(fN)} and the discount nets ${eur(dN)}.`}</desc>
        <line x1="36" x2="64" y1="12" y2="12" stroke="#2F5D62" strokeWidth="3" />
        <text x="70" y="16" fontSize="13" fontWeight="600" fill="#2F5D62">Fixed-cost lever</text>
        <line x1="230" x2="258" y1="12" y2="12" stroke="#A4472A" strokeWidth="3" strokeDasharray="7 5" />
        <text x="264" y="16" fontSize="13" fontWeight="600" fill="#A4472A">Discount</text>
        ...
        <text x={X0 - 6} y={py(0) + 4} textAnchor="end" fontSize="11.5" fill="#59606A">€0</text>
        {[0, 5, 10, 15].map((v) => (
          <text key={v} x={px(v)} y="218" textAnchor="middle" fontSize="12" fill="#59606A">{v} pp</text>
        ))}
        <text x="300" y="234" textAnchor="middle" fontSize="12.5" fill="#59606A">repeat-rate uplift, percentage points</text>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <p className="text-micro normal-case tracking-normal text-ash">How many percentage points the lever adds to the repeat rate.</p>
          <input id={`${uid}-u`} type="range" min={0} max={CH.umax} step={1} value={u} onChange={(e) => setU(Number(e.target.value))} className="range-accent" />
        </div>
        <div>
          <label htmlFor={`${uid}-b`} className="text-caption font-semibold">Existing repeat orders: <span className="tnum">{base}</span></label>
          <p className="text-micro normal-case tracking-normal text-ash">Orders the segment already places. The discount is paid on all of them.</p>
