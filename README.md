...
|---|---|---|
| `/route-1/` **Capstone** | **Materi A**: six cards, two per level, 10 min each (A1 buying motive → sales action, A2 the journey and the funnel read by its gaps · A3 three levers and what each nets, A4 segments and why a discount is weak · A5 goal → action → KPI and a governed system, A6 scale, sequence, owner, cadence, trigger). **Case File**: one case brief and **one task** of nine blocks on one page, in three thin parts (no stages, no handover panels, nothing locked): *Part 1 · Diagnose* (name the largest negative gap 1.3 and what the leak costs 1.4, Core; sort the touchpoints 1.1 and write down the funnel's figures 1.2, Optional), *Part 2 · Calculate* (net-impact grid 2.1 and one option per segment 2.3, Core; loss marks 2.2, Optional), *Part 3 · Decide* (3.1 spend the budget and set the start months; 3.2 what you cut or postpone, with a pickup point, and an owner, cadence and trigger for each funded item; both Core). | `1-{name}-day2-case-file.html` |
| `/route-2/` *optional* | The full Level 2: Materi B (B1–B4, 30 min) → Task 2 Calculation Note (15 min). Same blocks 2.1–2.3 as Part 2, plus block 2.4 (one option for both segments), which the Case File leaves out. | `2-{name}-day2-l2-calculation.html` |
| `/route-3/` *optional* | The full Level 3: Materi C (C1–C4, 30 min) → Task 3 Decision Memo (15 min), the memo assembling itself beside the questions. Blocks 3.1–3.5, which the Case File merges into its 3.1 and 3.2. | `3-{name}-day2-l3-memo.html` |

Route 1 material minutes: 6 × 10 = 60. Task minutes 8 + 13 + 20 (Core) + 10 (Optional) = about 50; the **Core** blocks alone (about 40 min) make a complete Case File, and the **Optional** blocks are never listed as missing. The optional routes keep their own minutes (Materi B and C 30 each, tasks 15 each).
...
**Shared answers.** Route 1 and the optional routes use the same store slices (`l1`, `l2`, `route3`), so an answer written in Route 1 is already in Route 2 and Route 3, and the other way round. The Case File's 3.2 field is the same field as Route 3's "measure you postponed" (3.5), and Route 3's separate "what was cut" (3.3) is not asked in Route 1.
...
26. **The capstone reuses the day's tasks instead of writing new ones.** Its blocks are answer blocks of Tasks 1 to 3 (same helps, keys and worked answers), each tagged **Core** or **Optional**: 1.1–1.4, 2.1–2.3 unchanged, and 3.1 and 3.2 built from Route 3's components (`components/capstone/Decision.tsx`). This is why the optional routes share answers with it. Core about 40 minutes, everything about 50, set in `components/capstone/Capstone.tsx`.
...
28. **The Case File counts Core blocks only.** Its one `missing` list (`caseMissing` in `lib/missing.ts`) spans the whole task and never lists 1.1, 1.2 or 2.2, and it has no block 2.4. Those blocks appear in the file only when the learner filled them in. The optional routes keep their own, stricter lists.
29. **No stages, no reference position.** The first capstone had three stages with a "Carried forward" panel and a "Use the reference position" button between them. The user found that too long and asked for one case and one task, so the stage panels, the stage strip, the handover panels and the reference position are gone (`applyReference` removed; `l1.refPosition` and `l2.refPosition` stay in the persisted shape, unused, so old blobs still load). Nothing in Route 1 is soft-locked any more: the lever calculator and the allocation grid are open from the start.
30. **Reset semantics.** Route 1 is the whole Case File, so "Reset Route 1" (and "Clear this route" in the mentor bar on Route 1) clears all three answer slices, which the optional routes share. Resetting Route 2 or 3 clears its own slice, and so also that part of Route 1.
31. **No live memo in Route 1.** Route 3 keeps its memo beside the questions. In the Case File the decision part is two blocks in one column and the preview above the export button shows the whole document.
32. **Persist version 5** (now 6, see 35). `ui.optionalRoutesShown`, `l1.refPosition` and `l2.refPosition` were added; `merge` fills them from the defaults for an older blob.

33. **Route 1 can be read in German (EN | DE), by explicit request.** CURRICULUM-GUIDE §1 says the CS days are English only; the user asked for a German version of everything on Route 1 for this day, so it is a day-level waiver (CLAUDE.md #18), recorded here. The switch sits in the top bar and only on `/route-1/`; the home page and the optional Routes 2 and 3 stay English (the switch is hidden there). It covers all learner-facing text of Route 1: material cards, diagrams, plain-words boxes, the German glossary (`data/glossaryDe.ts`), tasks, helps, clues, missing lists, the exported Case File and its preview, and the live memo.
34. **Mentor tools stay English.** The mentor bar, answer keys, worked answers (`lib/mentorGuide.ts`, `lib/answerKey.ts`) and the rubric panel are for the facilitator and are not translated. "Fill all model answers" does enter the free-text answers in German while the site is in German (`data/mentorKey.ts` getters), with German number formats.
35. **How the language works.** `ui.lang` ("en" | "de", persisted, version 6, migrated for older blobs) is the choice; `LangProvider` applies it only on Route 1 and re-mounts the tree when it changes. Text is written as `tt(en, de)` from `lib/lang.ts`; data files use getters so nothing is frozen at import. Values that answers store (owner, cadence and pickup options) stay the English constants and are only displayed in German (`ownerLabel`, `cadenceLabel`, `pickupLabel`), so checks, keys and both languages agree. Numbers follow the language (`1.234,5` and `12.345 €` in German). Free-text checks (a figure from your grid) read either format.
36. **References.** Only the explanatory part of a reference is translated (`refFull` in `data/references.ts`); authors, titles and journals are shown as published.
37. **Route 1 is one task, not three stages (user request, CLAUDE.md #29 updated).** The Case File used to have three stages with 13 blocks; it now has nine blocks in one continuous task. Block 2.4 was dropped; Route 3's five decision blocks became two (3.1 allocation plus start months, 3.2 what you cut or postpone plus governance), losing the separate cut field, the KPI-blind-spot questions (the live warning stays under the start months) and the live memo. Materi A is unchanged, apart from wording that named "Stage" or asked for one option for both segments. The final deliverable is the same Case File: diagnosis, calculation, decision.
...
| 2.4 (Route 2 only) | A4 | Not in the Case File; Route 2 keeps it |
| 3.1 Spend the budget, start months | A6 (cost shapes, add the costs, descope, cut the weakest evidence), A5 (baseline first) | Live total, budget bar, message naming the moves that close a gap · live sequencing warning under the start months · "Limits and reference points" in the brief |
| 3.2 Leave out, own | A6 (consequence in the material's terms, postpone with a pickup point, owner test, profiles, cadence, threshold) | "Left open" list · Show how to build the answer · Show the test questions (with the case's reference points for a trigger) · a trigger without a number is flagged |
...
38. **Optional blocks start collapsed.** Blocks 1.1, 1.2 and 2.2 show as one faint line ("Optional, about 3 min · … · click to show") so attention stays on the Core blocks; a click opens the block, "Hide" closes it, and a page-map click opens it first. The open state is session-only (`store/useOptionalOpen.ts`); the answers inside are persisted as usual. Nothing is locked.
